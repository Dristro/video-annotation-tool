...
def test_apply_light_theme_sets_light_window_color(qapp) -> None:
    apply_light_theme(qapp)
    window_color = qapp.palette().color(QPalette.ColorRole.Window)
    assert window_color.lightness() >= 128


def test_switching_between_themes_fully_replaces_palette(qapp) -> None:
    apply_dark_theme(qapp)
...
def test_apply_theme_dispatches_to_light(qapp) -> None:
    apply_theme(qapp, "light")
    assert qapp.palette().color(QPalette.ColorRole.Window).lightness() >= 128


def test_apply_theme_dispatches_to_dark(qapp) -> None:
    apply_theme(qapp, "dark")
    assert qapp.palette().color(QPalette.ColorRole.Window).lightness() < 128


def test_apply_theme_unknown_value_falls_back_to_dark(qapp) -> None:
