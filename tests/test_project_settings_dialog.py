...
def test_has_labels_and_scores_tabs(project) -> None:
...
def test_defaults_to_labels_tab(project) -> None:
    dialog = ProjectSettingsDialog(project)
    assert dialog._tabs.currentIndex() == 0


def test_initial_tab_scores_selects_scores_tab(project) -> None:
    dialog = ProjectSettingsDialog(project, initial_tab="scores")
    assert dialog._tabs.currentIndex() == 1


def test_labels_tab_reflects_project_labels(project) -> None:
    dialog = ProjectSettingsDialog(project)
    assert dialog.labels_widget._table.rowCount() == 1
    assert dialog.labels_widget._table.item(0, 1).text() == "goal"


def test_scores_tab_reflects_project_score_definitions(project) -> None:
    dialog = ProjectSettingsDialog(project)
